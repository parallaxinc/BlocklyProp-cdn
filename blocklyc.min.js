...
"-Os",function(a){if(a.error)console.log(a),alert("BlocklyProp was unable to compile your project:\n"+a.message+'\nIt may help to "Force Refresh" by pressing Control-Shift-R (Windows/Linux) or Shift-Command-R (Mac)');else{var d="compile"!==c?"\nDownload...":"";$("#compile-console").val($("#compile-console").val()+a.message+d);a.success&&a.binary&&b(a,e);document.getElementById("compile-console").scrollTop=document.getElementById("compile-console").scrollHeight}}):$.ajax({method:"POST",url:baseUrl+
...
function graph_new_data(a){if(-1<a.indexOf("ailed"))$("#graph-conn-info").html(a);else for(k=0;k<a.length;k++)if("\n"===a[k]&&(a[k]="\r"),"\r"===a[k]&&graph_data_ready){if(!graph_paused){graph_temp_data.push(graph_temp_string.split(","));var c=graph_temp_data.length-1}graph_timestamp_start&&0!==graph_timestamp_start||(graph_timestamp_start=ts,graph_start_playing&&(graph_timestamp_start-=graph_timestamp_restart,graph_timestamp_restart=0));0<c&&!graph_start_playing&&parseFloat(graph_temp_data[c][0])<
parseFloat(graph_temp_data[c-1][1])&&(graph_time_multiplier+=fullCycleTime);graph_start_playing=!1;if(!graph_paused){graph_temp_data[c].unshift(ts+graph_time_multiplier-graph_timestamp_start);var b=Math.round(1E4*graph_temp_data[c][0])/1E4+",";if("X"===graph_options.graph_type)for(var d=0,e=2;e<graph_temp_data[c].length;e+=2)b+=graph_temp_data[c][e]+","+graph_temp_data[c][e+1]+",",graph_data.series[d].push({x:graph_temp_data[c][e]||null,y:graph_temp_data[c][e+1]||null}),graph_temp_data[c][0]>graph_options.sampleTotal&&
graph_data.series[d].shift(),d++;else for(e=2;e<graph_temp_data[c].length;e++)b+=graph_temp_data[c][e]+",",graph_data.series[e-2].push({x:graph_temp_data[c][0],y:graph_temp_data[c][e]||null}),graph_temp_data[c][0]>graph_options.sampleTotal&&graph_data.series[e-2].shift();graph_csv_data.push(b.slice(0,-1).split(","));15E3<graph_csv_data.length&&graph_csv_data.shift()}graph_temp_string=""}else graph_data_ready?-1<"-0123456789.,\r\n".indexOf(a[k])&&(graph_temp_string+=a[k]):"\r"===a[k]&&(graph_data_ready=
!0)}function graph_reset(){graph_temp_data.length=0;for(var a=graph_csv_data.length=0;10>a;a++)graph_data.series[a]=[];graph&&graph.update(graph_data,graph_options,!0);graph_temp_string="";graph_timestamp_restart=graph_time_multiplier=graph_timestamp_start=0;graph_data_ready=!1}
